...
class TaskService:
    ...
    async def _get_or_404(self, user_id: int, task_id: int) -> TaskModel:
        task = await self.repo.get_task_by_user_and_id(user_id, task_id)
        if task is None:
            raise NotFoundError("Task", task_id)
        return task

    async def get_by_id(self, user_id: int, task_id: int) -> TaskRead:
        return TaskRead.model_validate(await self._get_or_404(user_id, task_id))
    ...
    async def create_task(self, user_id: int, data: TaskCreate) -> TaskRead:
        task = await self.repo.create(TaskModel(title=data.title, description=data.description, user_id=user_id))
        return TaskRead.model_validate(task)
    
    async def update_task(self, user_id: int, task_id: int, data: TaskUpdate) -> TaskRead:
    ...
    async def delete_task(self, user_id: int, task_id: int) -> None:
        await self.repo.delete(await self._get_or_404(user_id, task_id))
